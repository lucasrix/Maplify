...
class NotificationsViewController: ViewController, NotificationsCellDelegate {
    ...
    func loadItemsFromDB() {
        ...
        let realm = try! Realm()
        let notifications = Array(realm.objects(Notification).filter("action_user != nil AND (notificable_user != nil OR (notificable_storypoint != nil AND notificable_storypoint.reported == false) OR (notificable_story != nil AND notificable_story.storyPoints.@count > 0 AND notificable_story.reported == false))").sorted("created_at", ascending: false))
